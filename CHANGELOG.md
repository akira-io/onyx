...
- `osinfo.DeviceName()` returns a human-friendly machine name (macOS `ComputerName`, Windows `COMPUTERNAME`, Linux pretty hostname), falling back to `Hostname()` when no friendlier source is available.
